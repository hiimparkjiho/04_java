...
public interface InterProduct extends Serializable {
    ...
    * */
    ...
    // 인터페이스는 상수 필드만 가질 수 있기 때문에 모든 필드는 목시 적으로 public static final이 된다.
    int MIN_NUM = 10;

    // 인터페이스는 생성자를 가질 수 없다.
    // 인터페이스를 사용하는 목적은 주상 메서드를 묶어서 제공하기 위함이다.
    // 그렇기 때문에 INTERFACE를 생성하여 사용하는 일이 없이 어딘가에 상속하여 구현 후 사용하게 된다.
    //public InterProduct(){}

    // 추상메서드는 몸체를 갖이 않기 때문에 오류가 발생한다.
    //public abstract void nonStaticMethod()//;

    public abstract void nonStaticMethod();

    /*
    * 인터페이스 안에 작성한 메서드는 목시적으로 public abstract의 의미를 가진다. (다른 접근제한자 사용불가)

...
public class Seller {
    ...
    // 입력받은 타입의 재정이 된 price를 호출하여 하나의 매서드로 판매 가격을 작성할 수 있다.
    // 만약 위와 같이 하지 않는 경우 아래의 메서드 olCarSale(), electricCarSale() 각각 구현을 해야하며 이로 인해 별도의 속성을 갖게 된다.

    public void sale(Car[] cars){
        // 형변환을 하였다고 해서 heap 생성된 값이 변환되는 것은 아니며 주소를 잃는 것도 아니다.
        // 단지 해당 형에서 사용할 수 있는 기능만 사용할 수 있으며 그 외적은 기능은 쓰지 못하는 상태가 되는 것이다.
        // 이를 통해 다형성의 이점을 누릴 수 있게 된다.
        for (Car car: cars) {
            if(car instanceof ElectricCar){ // heap의 생성된 car의 자료형이 ElectroCar와 같은지를 비교함
                ElectricCar electricCar = (ElectricCar) car;    // 여기도
                System.out.println(electricCar.getBrand() + "사의 " + electricCar.getCarName() + "를 판매했습니다.");
    ...
    * 내연기관차(OldCar)를 판매하는 메서드와 전기차(ElectronicCar)를 판매하는 메서드를 따로
    * 만들어 제공을 해야할 것이다.
    * */
    
    public void oldCarSale(OldCar[] oldCar){
        for (OldCar car: oldCar) {
            System.out.println(car.getPrice() + "를 판매했습니다");
            // 판매 수수료는 10%이기 때문에 0.1곱함
...
//    }
    ...
    public String toString() {
        return name + "사원의 이번 달 월급은 " + result + " 만원입니다" ;
    }
}

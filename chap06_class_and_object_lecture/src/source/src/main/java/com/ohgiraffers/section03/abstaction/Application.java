...
public class Application {
    public static void main(String[] args){
        ...
        * 스스로 결정하고, 그 결정한 방법대로 처리할 명렁어들을 순차적으로 기술한 것을 메소드라고 한다.
        * 상호작용에 중요한 것은 메세지를 송수신하고 그 방법을 기술하여 메세지에 응답하는 상호작용을 설계하는 것이다.
        * 따라서 객체를 설계할 때는 속성은 부가적이며 적적한 책임을 가진 객체들간의 주고받는 메세지를 이용해 상호작용하는 것을 설계하는 것을 의미한다.
        * 즉, 필드보다는 메소드를 중점적으로 추상화 기법을 작용하여 객체를 설계하고, 그 객체를 구현하는 방법으로 설계한대로 클래스를 작성하는 것이다.
        * 이것을 추상화라고 부른다.
        * */
    ...
    /*
    * 추상화란 객체지향 프로그래밍(oop)와 소프트웨어 공학에서 중요한 개념 중 하나이며, 추상화는 복잡한 현실세계의 개념, 데이터, 시스템 또는 프로세스를 단순화하거나
    * 요약하여 핵심적인 특징을 강조하고, 불필요한 세부 사항을 숨기는 과정이다.
    *
    ...
    *
    * 은닉화 : 불필요한 세부 사항을 숨기고 필요한 정보만을 노출한다. 이것은 정보은닉과 관련이 있으며 객체의 내부 구형을 외부로부터 숨겨 보하하고 필요한 인터페이스(메소드)만 노출하는 것을 의미한다.
    *
    * 모델링 : 추상화는 현실 세계의 개념을 소프트웨어 모델로 변환하여 이 모델은 소프트웨어 개발에서 사용된다. 복잡한 현실 세계를 이해하고 해결책을 제공하는 도구로 사용된다.
    *
    * 객체지향 프로그래밍 : 추상화는 객체지향 프로그래밍의 핵심 개념 중 하나로 클래스와 객체를 사용하여 추상화를 통해 현실 세계의 개념을 모델링하고 이를 소프트웨어로 구현한다.
    * 클래스는 추상 데이터 타입을 정의하고, 객체는 클래스의 인스턴스로 신제 데이터와 메서드를 갖는다.
    *
    * 인터페이스 : 인터페이스는 추상화의 한 형태로 객체가 특정 동작을 수행하는 방법을 정의하는 역할을 하며 인터페이스는 추상 메서드를 포함하여 클래스에서 이를 구현함으로써 필요한 동작을 정의할 수 있다.
    * */
}
